...
import { getRecentlyPlayedAlbums, refreshAccessTokenWithExpiresIn } from "@/lib/spotify";
...
export async function GET(request: Request) {
  ...
  try {
    const albums = await getRecentlyPlayedAlbums(token, 50);
    ...
    console.error("Spotify recently played failed", err);
